---
title: "Support Vector Machine with Radial Basis Kernel"
format: 
  html:
    theme: cosmo
    smooth-scroll: true
...
`Caution!` 함수 `train()`에 옵션 `method = "svmRadial"`을 입력하면 Package `"kernlab"`의 함수 `ksvm()`를 이용하여 Support Vector Machine을 수행한다. 해당 함수는 Kernel 함수가 `Radial Basis`일 때, 최적의 `sigma` 값을 자동으로 찾아준다.  
